    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="text-2xl font-bold text-primary-dark tracking-tight">tumbledry</span>
        </Link>

        {/* Desktop Menu */}
        <div className="hidden md:flex items-center gap-8">
          <Link href="#services" className="text-text hover:text-primary transition-colors font-medium">Services</Link>
          <Link href="#process" className="text-text hover:text-primary transition-colors font-medium">How it Works</Link>
          <Link href="#pricing" className="text-text hover:text-primary transition-colors font-medium">Pricing</Link>
          <Link href="#locations" className="text-text hover:text-primary transition-colors font-medium">Stores</Link>
        </div>
        ...
        <div className="hidden md:flex items-center gap-4">
          {!user ? (
            <Link 
              href="/login"
              className="flex items-center gap-2 text-text font-medium hover:text-primary transition-colors border border-gray-200 px-4 py-2 rounded-full shadow-sm hover:shadow"
            >
              <LogIn size={16} />
              <span>Login</span>
            </Link>
            ...
            <div className="flex items-center gap-3">
              <Link href="#track" className="flex items-center gap-1.5 text-primary bg-blue-50 px-3 py-1.5 rounded-full text-sm font-bold hover:bg-blue-100 transition-colors">
          ...
          <Link href="#pickup" className="bg-primary hover:bg-primary-dark text-white px-6 py-2.5 rounded-full font-medium transition-all shadow-sm hover:shadow-md transform hover:-translate-y-0.5">
            Schedule Pickup
          </Link>
        </div>
        ...
        {isMobileMenuOpen && (
          ...
          >
            <div className="flex flex-col p-4 space-y-4">
              ...
              {user && (
                <Link href="#track" className="p-3 text-lg font-bold text-primary bg-blue-50/50 hover:bg-blue-100 rounded-lg flex items-center gap-2" onClick={() => setIsMobileMenuOpen(false)}>
                  <Package size={20} /> Track Active Order
                </Link>
              )}

              <div className="pt-4 border-t border-gray-100 flex flex-col gap-3">
                <Link href="#pickup" onClick={() => setIsMobileMenuOpen(false)} className="bg-primary text-white p-3 rounded-lg font-medium shadow-sm w-full text-center block">
                  Schedule Pickup
                </Link>
                {user && (
